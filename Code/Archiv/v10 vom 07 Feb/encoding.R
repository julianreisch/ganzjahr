encode<-function(el,r){
  ...
  el_enc<-el[which(el$beenparent==0),] # Encode nur Systemtrassen, die Blätter sind im Baum der geteilten Trassen
  ...
  rep<-matrix(-1,nrow(r),nrow(el_enc))
  colnames(rep)<-el_enc$id
  rownames(rep)<-r$id
  
  for(i in 1:nrow(r)){
    legale_trassen<-as.integer(unlist(strsplit(r$res[i], split=", ")))
    
    for(k in 1:nrow(el_enc)){
      
      # Finde erstmögliche Subsystemtrasse als Repräsentanten
      if((el_enc$id[k] %in% legale_trassen) &
         length(intersect(which(r[i,11:(10+n)]==1),which(el_enc[k,7:(6+n)]==1)))!=0){
        
        # Suche nach allen anderen Subsystemtrassen, die diesen Repräsentanten haben (also denselben (ur-)parent haben) 
        for(j in 1:nrow(el_enc)){
          if(el_enc$parent[j]!=0 & el_enc$parent[j]==el_enc$parent[k] & 
             (el_enc$id[j] %in% legale_trassen) &
             length(intersect(which(r[i,11:(10+n)]==1),which(el_enc[j,7:(6+n)]==1)))!=0){
            rep[i,j]<-el_enc$id[k]
          }
        }
        rep[i,k]<-0
  ...
  for(i in 1:length(unique(r$fahrlage))){
    if(length(unique(r$partition[which(r$fahrlage==unique(r$fahrlage)[i])]))>0){
      indx<-1:length(unique(r$partition[which(r$fahrlage==unique(r$fahrlage)[i])]))
    }else{indx<-1}
  ...
  for(i in 1:length(unique(r$fahrlage))){
    ...
    for(k in 1:length(unique(r$partition[which(r$fahrlage==fahrlage)]))){
      part<-unique(r$partition[which(r$fahrlage==fahrlage)])[k]
