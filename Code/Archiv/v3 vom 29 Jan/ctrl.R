...
source("Tests/unit test patrick.R")
...
while(all(res$solution==0)){
  ...
  if(nrow(el)==nrow_el_alt){
    ...
    if(terminate==TRUE){
      ...
      print(paste("The winner is: Fahrlage",fahrlage_gewinner," und Partition",partition_gewinner))
    ...
    if(terminate==FALSE){
      ...
      b<-enc$b
      omega<-enc$omega
      r<-max_SAT(el,r,makro,A,dir,b,omega)
      print("Löse MaxSAT.")
...
for(i in 1:nrow(unique(r[which(r$valid==1),c("id","partition","fahrlage")]))){
  colnames(solution)[count]<-paste("x_(",unique(r[which(r$valid==1),c("id","partition","fahrlage")])[i,3],",",
                                   ...
                                   unique(r[,c("id","partition","fahrlage")])[i,1],sep="")
  count<-count+1
}
...
print(paste("Es wurden ",sperren_count," Partitionsvergleiche durchgeführt.",sep=""))
print(paste("Es wurden ",ablehnen_count-length(unique(r$fahrlage))," Fahrlagen abgelehnt.",sep=""))
el[which(el$beenparent==0),][which(el[which(el$beenparent==0),]$id %in% c(49,71,72,77,78)),]