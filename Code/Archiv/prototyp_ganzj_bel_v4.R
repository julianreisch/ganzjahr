...
## Check ob Aufteilen der betroffenen Systemtrasse ausreicht
for(k in 1:length(mikro)){
  if(all(sapply(r[makro,2:3],sum)-sapply(el[mikro[k],5:6],sum)<=0)){
    for(i in makro){
      s<-data.frame(id=nrow(el)+1,von=el[mikro[k],]$von,bis=el[mikro[k],]$bis,weight=el[mikro[k],]$weight,
                      tag1=r$tag1[i],tag2=r$tag2[i])
      el<-rbind(el,s)
    }
    if(sum(sapply(r[makro,2:3],sum)-sapply(el[mikro[k],5:6],sum))<0){
      if(as.integer(sapply(r[makro,2:3],sum)-sapply(el[mikro[k],5:6],sum))[1]==1){
        tag1<-1
      }else{tag1<-0}
      if(as.integer(sapply(r[makro,2:3],sum)-sapply(el[mikro[k],5:6],sum))[2]==1){
        tag2<-1
      }else{tag2<-0}
