        ...
        while(p <= length(unique(r$id[which(r$fahrlage==makro[i] & r$valid==1)])) & terminate==FALSE){
          while(q <= length(unique(r$id[which(r$fahrlage==makro[j] & r$valid==1)])) & terminate==FALSE){
            ...
            if(mikro[k] %in% intersect(as.integer(unlist(strsplit(r$res[p_index], split=", "))),
              ...
              print(sapply(r[c(p_index,q_index),(11:(10+n))[gueltig]],sum))
              print(el[which(el$id==mikro[k]),(7:(6+n))[gueltig]])
              
              print("Dim:")
              print(dim(r[c(p_index,q_index),(11:(10+n))[gueltig]]))
              
              # Hat das Paar von Fahrlagenvarianten keinen Schnitt auf Gültigkeitszeitraum der Trasse?
              #if(is.null(dim(r[c(p_index,q_index),(11:(10+n))[gueltig]]))){
               # schnitt_var<-sum(r[c(p_index,q_index),(11:(10+n))[gueltig]])
              #}else{
                schnitt_var<-sapply(r[c(p_index,q_index),(11:(10+n))[gueltig]],sum)
              #}
              if(all(schnitt_var-el[which(el$id==mikro[k]),(7:(6+n))[gueltig]]<=0)){
