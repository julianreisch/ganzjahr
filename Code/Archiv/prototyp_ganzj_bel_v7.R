...
for(i in 1:n){
  r<-cbind(r,data.frame(tag=rep(1,R)))
}
colnames(r)[4:(n+3)]<-tage
  ...
  while(k <= length(mikro) & terminate==FALSE){
    while(i <= length(makro)-1 & terminate==FALSE){
      while(j <= length(makro) & terminate==FALSE){
        ...
        if(all(sapply(r[makro[c(i,j)],4:(3+n)],sum)-sapply(el[mikro[k],6:(5+n)],sum)<=0)){
          ...
          if(!all(sapply(r[makro[c(i,j)],4:(n+3)],sum)-sapply(el[mikro[k],6:(5+n)],sum)>=0)){
            tage<-which(sapply(r[makro[c(i,j)],4:(n+3)],sum)-sapply(el[mikro[k],6:(5+n)],sum)<0)
            s_rest<-el[1,]
            s_rest[1,1:5]<-c(nrow(el)+1,el[mikro[k],]$von,el[mikro[k],]$bis,el[mikro[k],]$weight,mikro[k])
