...
while(res$status!=0){
  ...
  ## Falls der Makrokonlikt nur entsteht, weil eine Fahrlage noch überhaupt keinen Weg hat, dann suche sofort nach
  ## bestmöglicher Partitionierung:
  if(length(benutzteSystemtrassen)==1){
    ...
    for(i in 1:length(makro)){
      ...
      for(j in 1:length(partitionen_ids)){
        ...
        ex<-extend_solution_spaces(el,r,varianten_indx,n) # Mach Wegesuche für alle Varianten der Partition j der Fahrlage i
        r_candid<-ex$r
    ...
    ## Falls keine neuen Wege für keine Partition gefunden und der Konflikt demnach nicht aufgelöst wurde, lehne Fahrlagen ab
    if(terminate==FALSE){
    ...
    if(nrow(el)==nrow_el_alt){
      ...
      for(i in 1:length(makro)){
        ...
        for(j in 1:length(partitionen_ids)){
          ...
          # Prüfe bei neuen Partitionen, dass wirklich jede Fahrlagenvariante einen Weg hat
          #alleWege<-TRUE
        ...
        ## Ergänze r um neuen Weg/ neue Partition des Gewinners pro Fahrlage im Makrokonflikt
        #  r<-r_gewinner
        #  indx_update<-which(r$partition==partition_gewinner & r$fahrlage==fahrlage_gewinner)
        #  r$valid[indx_update]<-rep(1,length(indx_update))
        #  print(paste("The winner is: Fahrlage ",fahrlage_gewinner," und Partition ",partition_gewinner,sep=""))
